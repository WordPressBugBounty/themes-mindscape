...
(function ($) {
  var $notice_container = $(".mindscape-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.mindscape_admin) {
    return;
  }

  var _window$mindscape_adm = window.mindscape_admin,
      builderStatusData = _window$mindscape_adm.builderStatusData,
      getStartedData = _window$mindscape_adm.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.mindscape-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".mindscape-admin-big-notice").addClass("processing");
      $(".mindscape-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".mindscape-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.mindscape-admin-big-notice");
    });
  }
  ...
  var $notice_root = $notice_container.closest(".mindscape-admin-big-notice");
  var $custom_close_button = $notice_root.find(".mindscape-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".mindscape-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.mindscape_admin.showOverlay = showOverlay;
})(jQuery);
})();
